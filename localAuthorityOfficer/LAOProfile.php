...
/* ==========================================================================
   ...
   Triggered by #profilePicForm (multipart/form-data, plain POST,
   full page reload) — distinguished by the presence of $_FILES['profile_picture']
   ========================================================================== */
if (isset($_FILES['profile_picture'])) {

    if ($_FILES['profile_picture']['error'] !== UPLOAD_ERR_OK) {
        header('Location: LAOprofileForm.php?pic=error&msg=' . urlencode('No file was uploaded, or the upload failed.'));
        exit;
    }
    ...
    if (!in_array($mimeType, $allowedMimeTypes, true)) {
        header('Location: LAOprofileForm.php?pic=error&msg=' . urlencode('Invalid file type. Please upload a PNG, JPG, or WEBP image.'));
        exit;
    }

    if ($file['size'] > $maxSize) {
        header('Location: LAOprofileForm.php?pic=error&msg=' . urlencode('File is too large. Maximum size is 5MB.'));
        exit;
    }

    // Delegate to User class method
    $user = new User();
    $result = $user->updateProfilePicture($userID, $file);

    if (isset($result['success']) && $result['success'] === true) {
        header('Location: LAOprofileForm.php?pic=success');
    } else {
        $msg = $result['message'] ?? 'Failed to update profile picture.';
        header('Location: LAOprofileForm.php?pic=error&msg=' . urlencode($msg));
    }
    exit;
}
...
try {
    ...
    if ($updated) {
        // Keep session in sync so topbar/sidebar reflect changes immediately
