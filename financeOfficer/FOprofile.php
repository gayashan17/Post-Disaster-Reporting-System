...
if (isset($_FILES['profile_picture'])) {
    ...
    if ($_FILES['profile_picture']['error'] !== UPLOAD_ERR_OK) {
        header('Location: FOprofileForm.php?pic=error&msg=' . urlencode('No file was uploaded, or the upload failed.'));
        exit;
    }
    ...
    if (!in_array($mimeType, $allowedMimeTypes, true)) {
        header('Location: FOprofileForm.php?pic=error&msg=' . urlencode('Invalid file type. Please upload a PNG, JPG, or WEBP image.'));
        exit;
    }

    if ($file['size'] > $maxSize) {
        header('Location: FOprofileForm.php?pic=error&msg=' . urlencode('File is too large. Maximum size is 5MB.'));
        exit;
    }
    ...
    if (isset($result['success']) && $result['success'] === true) {
        header('Location: FOprofileForm.php?pic=success');
    } else {
        $msg = $result['message'] ?? 'Failed to update profile picture.';
        header('Location: FOprofileForm.php?pic=error&msg=' . urlencode($msg));
    }
    exit;
}
